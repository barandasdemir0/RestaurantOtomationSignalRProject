...
namespace Project.WebUI.Controllers
{
    ...
    public class MenuTablesController : Controller
    {
        ...
        public async Task<IActionResult> MenuTableCreate(CreateMenuTableDto createMenuTableDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7240/api/MenuTables", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync($"https://localhost:7240/api/MenuTables/{id}");
            return RedirectToAction("Index"); // artık view aramıyor
        }


        [HttpGet]
        public async Task<IActionResult> MenuTableUpdate(int id)
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7240/api/MenuTables/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> MenuTableUpdate(UpdateMenuTableDto updateMenuTableDto)
        {
            updateMenuTableDto.Status = false;
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(updateMenuTableDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7240/api/MenuTables", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
